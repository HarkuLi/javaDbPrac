...
public class TestInterestGetPage {
	...
	private ArrayList<IntModel> page1List;
	private ArrayList<IntModel> page2List;
	private IntModel filterMatchNothing;
	
	@Mock
	private IntService interestService;
	
	@InjectMocks
	private IntRestController intRestController;
	...
	public void noOneMatchFilter() throws Exception {
		...
		when(interestService.getPage(anyInt(), argThat(filter -> filter.getName().equals(filterMatchNothing.getName())
													&& filter.getState() == filterMatchNothing.getState())))
			.thenReturn(new ArrayList<IntModel>());
	...
	private void setTestData() {
		page1List = new ArrayList<IntModel>();
		page1List.add(RandomData.genInterest());
		page2List = new ArrayList<IntModel>();
		page2List.add(RandomData.genInterest());
		filterMatchNothing = RandomData.genInterest();
	}
	
	private void setStubs() {
		when(interestService.getTotalPage(any(IntModel.class))).thenReturn(totalPage);
		when(interestService.getPage(eq(1), any(IntModel.class))).thenReturn(page1List);
		when(interestService.getPage(eq(2), any(IntModel.class))).thenReturn(page2List);
	}
}
